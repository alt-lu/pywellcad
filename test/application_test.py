...
try:
    obj_wcad.new_borehole(f"{path}\Well1.wdt")
except:
	PrintException("NewBorehole")
   
   
try:
    obj_wcad.close_borehole(False)
except:
	PrintException("CloseBorehole")

    
try:
    obj_wcad.open_borehole(f"{path}\Well1.wcl")
except:
	PrintException("OpenBorehole")
    
    
try:
    doc_count = obj_wcad.borehole_count
except:
...
try:
    ...
    obj_bhole.name = "Master Test 2"
except:
	PrintException("GetBorehole")


try:
    obj_bhole = obj_wcad.file_import(f"{path}\Well1.las", False)
    obj_wcad.close_borehole(False)
except:
...
try:
    obj_bhole = obj_wcad.multi_file_import(f"{path}\Well1_Strata.waq,\
                                           {path}\Well1_Strata.waq",
                                           False)
    obj_wcad.close_borehole(False)
except:
