...
class TestBorehole(unittest.TestCase, ExtraAsserts, SamplePath):
    ...
    def test_comp_wave_velocity(self):
        # Try to compute the compressional wave velocity using the constant method
        config = "Method=0, OutputUnit=m/s, ConstantOrLog=1700"
        output_log = self.rock_mechanics.comp_wave_velocity(config)
        self.assertIsInstance(output_log, wellcad.com.Log)
        self.rock_mechanics.remove_log(output_log.name)

        # Try to compute the compressional wave velocity using the Christensen and Stanley method
        config = "Method=1, OutputUnit=m/s, Density=RHOB, DensityUnit=g/cc"
        output_log = self.rock_mechanics.comp_wave_velocity(config)
        self.assertIsInstance(output_log, wellcad.com.Log)
        self.rock_mechanics.remove_log(output_log.name)

        # Try to compute the compressional wave velocity using the Lindseth method
        config = "Method=2, OutputUnit=m/s, Density=RHOB, DensityUnit=g/cc"
        output_log = self.rock_mechanics.comp_wave_velocity(config)
        self.assertIsInstance(output_log, wellcad.com.Log)
        self.rock_mechanics.remove_log(output_log.name)

        # Try to compute the compressional wave velocity using the Christensen method
        config = "Method=3, OutputUnit=m/s, Density=RHOB, DensityUnit=g/cc"
        output_log = self.rock_mechanics.comp_wave_velocity(config)
        self.assertIsInstance(output_log, wellcad.com.Log)
        self.rock_mechanics.remove_log(output_log.name)

        # Try to compute the compressional wave velocity using the Yasar and Erdogan method
        config = "Method=4, OutputUnit=m/s, Density=RHOB, DensityUnit=g/cc"
        output_log = self.rock_mechanics.comp_wave_velocity(config)
        self.assertIsInstance(output_log, wellcad.com.Log)
        self.rock_mechanics.remove_log(output_log.name)
