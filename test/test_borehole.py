...
class TestBorehole(unittest.TestCase, ExtraAsserts, SamplePath):
    ...
    def test_friction_angle(self):
        # Try to compute the friction angle using the constant method
        config = "Method=0, ConstantOrLog=20"
        output_log = self.rock_mechanics.friction_angle(config)
        self.assertIsInstance(output_log, wellcad.com.Log)
        self.rock_mechanics.remove_log(output_log.name)

        # Try to compute the friction angle using the Weingarten and Parkins method
        config = "Method=1, Porosity=0.3"
        output_log = self.rock_mechanics.friction_angle(config)
        self.assertIsInstance(output_log, wellcad.com.Log)
        self.rock_mechanics.remove_log(output_log.name)

        # Try to compute the friction angle using the Ameen method
        config = "Method=2, Porosity=0.3"
        output_log = self.rock_mechanics.friction_angle(config)
        self.assertIsInstance(output_log, wellcad.com.Log)
        self.rock_mechanics.remove_log(output_log.name)

        # Try to compute the friction angle using the Lal method
